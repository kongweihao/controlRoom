...
class Rota extends Auth{ // 上线用的
	...
	public function updateRotaOrdinary(Request $request,RotaModel $rota){
		...
		for($i = 0; $i < sizeof($data['data']); $i++){
			...
			if( // 判断前一晚是否夜班
				sizeof(Db::table('rota')->where(['time_stamp'=>$yesterdayTimeStamp,'member_name'=>$data['data'][$i]['replacement'],'is_night'=>1])->select())
				&&
				// 且被检测班次为非夜班
				Db::table('rota')->where(['time_stamp'=>$data['data'][$i]['rota_time_stamp'],'monitor_post_name'=>$data['data'][$i]['monitor_post_name']])->find()['is_night'] == 0){
				// 记录夜白连上信息
				$rs['code'] = 2;
				$brr = array();
